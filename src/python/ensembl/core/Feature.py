...
class Feature():
    ...
    def __init__(self,
        ...
        if start and end:
            ...
            if end+1 < start and slice and not slice.is_circular():
                raise ValueError(
                    f'Start {start} must be less than or equal to end+1 {end+1}')
    ...
    def length(self) -> int:
        if self._end < self._start:
            ...
            if self._slice.is_circular():
                len = self._slice.seq_region_length - \
                    (self._start-self._end) + 1
                return len
            raise Exception(
                f'Cannot determine length of non-circular feature where start > end')
    ...
    def feature_slice(self) -> Slice:
        if not self._slice:
            warnings.warn(
                f"Cannot obtain Feature_Slice for feature without attached slice", UserWarning)
            return None
